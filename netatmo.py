...
_GETSTATIONSDATA_REQ = _BASE_URL + "api/gethomecoachsdata"
    ...
    def access_token(self):
        ...
        if self._access_token is None:
            ...
            if resp is None:
                return False
            if "error" in resp:
                print("errdor", resp["error"], _AUTH_REQ)
                return None
            ...
            resp = post_request(_AUTH_REQ, post_params)
            print(post_request(_AUTH_REQ, post_params))
            if resp is None:
                return False
            if "error" in resp:
                print("erfror", resp["error"], _AUTH_REQ)
                return None
...
def dl_csv(hc, csv_file, device_id, module_id, fields, date_end=None):
    """
...
def fetch(rc_file_or_dict=None):
    ...
    """
    hc = HomeCoach(rc_file_or_dict)
    if not hc.get_data():
        return
    station = hc.station_by_name()
...
def self_test(args):
    ...
    """
    hc = HomeCoach(args.rc_file)
    ok = hc.get_data()
    if sys.stdout.isatty():
        if ok:
            print("netatmo.py %(mail)s : OK" % hc.user)
        else:
...
def dump(args):
    ...
    station = hc.station_by_name(args.device)

    if station is None:
        return

    # TODO
    # print("user %s" % (hc.user['mail']))
    # pprint.pprint(hc.user)
    ...
    measures = hc.get_measure(date_begin=half_hour, device_id=station["_id"])
...
def list_stations(args):
    ...
    hc = HomeCoach(args.rc_file)
    hc.get_data("*")
    for i, device in enumerate(hc.devices):
        print(
            i + 1,
            "station",
            device["_id"],
...
def action_config(args):
    ...
    if n >= 1 and n < 4:
        ...
        hc.load_credentials()
        if n == 4:
            hc.auth(args.client_id, args.client_secret, args.username, args.password)
        if args.device is not None:
            hc.set_default_station(args.device)
        hc.save_credentials()
    ...
    hc.load_credentials()
    print("username:", hc.username)
    print("password:", hc.password)
    print("client_id:", hc.client_id)
    print("client_secret:", hc.client_secret)
    print("default_device_id:", hc.default_device_id)
